...
const Index = () => {
  ...
  const filteredBrands = Brands.filter(brand => {
    ...
    return (
      brand.categories.some(tag => selectedTags.includes(tag)) ||
      brand.attributes?.some(tag => selectedTags.includes(tag)) ||
      brand.certifications?.some(tag => selectedTags.includes(tag))
    );
  });

  const BrandBoxesEmpty = () => {
    return <div className="w-1/4 bg-white p-2 shadow rounded-lg "></div>;
  };
  ...
  const BrandBoxes = () => {
    ...
    return (
      <div className="w-1/4 bg-white p-2 shadow rounded-lg ">
        ...
        {Brands.flatMap(brand => brand.categories)
          ...
          .map(tag => (
            <div key={tag} className="flex items-center ml-6">
              ...
              <label
                htmlFor={tag}
                className={`text-lg ${selectedTags.includes(tag) ? 'font-bold' : ''}`}
              >
                {tag}
              </label>
            </div>
        ...
        {Brands.flatMap(brand => brand.attributes || [])
          .filter((tag, index, self) => self.indexOf(tag) === index)
          .map(tag => (
            <div key={tag} className="flex items-center ml-6">
              <input
        ...
        <h2 className="text-xl font-bold mb-4 mt-10">Certifications</h2>
        {Brands.flatMap(brand => brand.certifications || [])
          .filter((tag, index, self) => self.indexOf(tag) === index)
          .map(tag => (
            <div key={tag} className="flex items-center ml-6">
              <input
          ...
          {filteredBrands.map((brand, index) => (
            <div key={index} className="bg-white p-4 shadow rounded-lg m-4 opacity-90">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                {brand.certifications?.map((tag, index) => (
                  <span
                    key={index}
                    className="bg-yellow-100 font-bold	text-black-600 text-xs px-2 py-1 mr-2"
                  >
                    {tag}
                  </span>
                ))}
